...
namespace PatuhService.Controllers
{
    public class ArticleController : ApiController
    {
        ...
        public IList<ArticleViewModel> Get(string UserID)
        {
            ...
            using (PatuhEntities db = new PatuhEntities())
            {
                IList<TrArticle> articles = db.TrArticles.Where(x => x.cCreated == UserID && x.Category == "ARTICLE").ToList();
        ...
        public object Get(long id)
        {
            ...
            using (PatuhEntities db = new PatuhEntities())
            {
                ...
                if (article == null) article = new TrArticle();
                ...
                articleViewModel.GPSLong = article.GPSLong;
                articleViewModel.GPSLat = article.GPSLat;
                articleViewModel.ImageIds =  db.TrImageAttachments.Where(x => x.HeaderId == id).Select(y=> y.Id).ToArray();
            }
